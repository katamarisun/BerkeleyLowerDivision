...
def balanced(m):
    ...
    if is_weight(m):
        ...
        left = sides(m)[0]
        right = sides(m)[1]
        left_weight = length(left) * total_weight(end(left))
        right_weight = length(right) * total_weight(end(right))
        return left_weight == right_weight and balanced(end(left)) and balanced(end(right))
