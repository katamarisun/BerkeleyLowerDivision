...
def put_piece(board, max_rows, column, player):
    ...
    """
    "*** YOUR CODE HERE ***"
    index_rows = max_rows - 1
    while index_rows >= 0:
        if get_piece(board, index_rows, column) == '-':
            board[index_rows] = replace_elem(board[index_rows], column, player)
            return index_rows, board
        index_rows -= 1
    return -1, board
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    j = 0
    while j < max_rows:
        if get_piece(board, j, col) == player:
            count += 1
            j += 1
        else:
            count = 0
