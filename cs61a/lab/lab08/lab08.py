...
def make_to_string(front, mid, back, empty_repr):
    ...
    def style(lnk):
        if lnk is Link.empty:
            ...
            return front + str(lnk.first) + mid + style(lnk.rest) + back
    return style
...
def add_trees(t1, t2):
    ...
    4
      ...
      5
    """
    if not t1.branches and not t2.branches:
        return Tree(t1.root + t2.root)
    elif not t1.branches:
        return Tree(t1.root + t2.root, t2.branches)
    elif not t2.branches:
        return Tree(t1.root + t2.root, t1.branches)
    if len(t1.branches) < len(t2.branches):
        t1.branches.extend([Tree(0) for i in range(len(t2.branches) - len(t1.branches))])
    elif len(t1.branches) > len(t2.branches):
        t2.branches.extend([Tree(0) for i in range(len(t1.branches) - len(t2.branches))])
    return Tree(t1.root + t2.root, [add_trees(b1, b2) for b1, b2 in zip(t1.branches, t2.branches)])
