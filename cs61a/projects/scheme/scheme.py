...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError
        val = []
        vals.map(lambda x: val.append(x))
        formal = []
        formals.map(lambda x: formal.append(x))
        for pair in zip(formal, val):
            child.define(pair[0], pair[1])
        # END PROBLEM 11
        return child
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        env.define(expressions.first, expressions.second.map(lambda x: scheme_eval(x, env)).first)
