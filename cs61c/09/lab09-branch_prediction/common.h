...
long long int sum_simd(unsigned int vals[NUM_ELEMS]) {
	...
	for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
		...
		for (int i = 0; i < NUM_ELEMS / 4 * 4; i += 4) {
			__m128i inner = _mm_loadu_si128((__m128i *) (vals + i));
			__m128i compare = _mm_cmpgt_epi32(inner, _127);
			inner = _mm_and_si128(compare, inner);
			sum = _mm_add_epi32(inner,sum);
		} 
...
long long int sum_simd_unrolled(unsigned int vals[NUM_ELEMS]) {
	...
	for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
		...
		for (int i = 0; i < NUM_ELEMS / 16 * 16; i += 16) {
			__m128i inner1 = _mm_loadu_si128((__m128i *) (vals + i));
			__m128i compare1 = _mm_cmpgt_epi32(inner1, _127);
			inner1 = _mm_and_si128(compare1, inner1);
			sum = _mm_add_epi32(inner1,sum);
			inner1 = _mm_loadu_si128((__m128i *) (vals + i + 4));
			compare1 = _mm_cmpgt_epi32(inner1, _127);
			inner1 = _mm_and_si128(compare1, inner1);
			sum = _mm_add_epi32(inner1,sum);
			inner1 = _mm_loadu_si128((__m128i *) (vals + i + 8));
			compare1 = _mm_cmpgt_epi32(inner1, _127);
			inner1 = _mm_and_si128(compare1, inner1);
			sum = _mm_add_epi32(inner1,sum);
			inner1 = _mm_loadu_si128((__m128i *) (vals + i + 12));
			compare1 = _mm_cmpgt_epi32(inner1, _127);
			inner1 = _mm_and_si128(compare1, inner1);
			sum = _mm_add_epi32(inner1,sum);
		} 
